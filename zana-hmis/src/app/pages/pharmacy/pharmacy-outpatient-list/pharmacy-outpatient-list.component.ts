...
@Component({
  selector: 'app-pharmacy-outpatient-list',
  templateUrl: './pharmacy-outpatient-list.component.html',
  styleUrls: ['./pharmacy-outpatient-list.component.scss']
})
export class PharmacyOutpatientListComponent {
  ...
  async loadOutpatientList(){    
    ...
    this.spinner.show()
    await this.http.get<IPatient[]>(API_URL+'/patients/get_pharmacy_outpatient_list', options)
    .pipe(finalize(() => this.spinner.hide()))
    .toPromise()
    .then(
      data => {
