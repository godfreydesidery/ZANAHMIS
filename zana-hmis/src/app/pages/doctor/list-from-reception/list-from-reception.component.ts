...
@Component({
  selector: 'app-list-from-reception',
  templateUrl: './list-from-reception.component.html',
  styleUrls: ['./list-from-reception.component.scss']
})
export class ListFromReceptionComponent implements OnInit {
  ...
  async loadListFromReception(clinicianIid : any){
    ...
    this.spinner.show()
    await this.http.get<IConsultation[]>(API_URL+'/patients/load_pending_consultations_by_clinician_id?clinician_id='+clinicianIid, options)
    .pipe(finalize(() => this.spinner.hide()))
    .toPromise()
    .then(
      data => {
